    ...
    async def execute(self, func: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(self.config.max_attempts):
            try:
                ...
                if attempt > 0:
                    logger.info(f"Succeeded after {attempt + 1} attempts (strategy: {self.config.strategy.value})")
                ...
                logger.warning(
                    f"Attempt {attempt + 1}/{self.config.max_attempts} failed: {e}. Retrying in {delay:.2f}s..."
                )
...
class CircuitBreaker:
    ...
    async def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if self.state == CircuitState.OPEN:
            if self._should_attempt_reset():
                ...
                raise CircuitBreakerError(f"Circuit breaker '{self.name}' is OPEN. Last error: {self.last_error}")
    ...
    def _on_success(self):
        ...
        if self.state == CircuitState.HALF_OPEN:
            ...
            if self.success_count >= self.config.success_threshold:
                logger.info(f"Circuit breaker '{self.name}' HALF_OPEN -> CLOSED ({self.success_count} successes)")
                self.state = CircuitState.CLOSED
                self.last_error = None
    ...
    def _on_failure(self, error: Exception):
        ...
        if self.state == CircuitState.HALF_OPEN:
            logger.warning(f"Circuit breaker '{self.name}' HALF_OPEN -> OPEN (failed during reset)")
...
class RateLimiter:
    ...
    def get_status(self) -> dict:
        ...
        active_calls = [call_time for call_time in self._calls if call_time > window_start]
...
class FallbackManager:
    ...
    async def execute_with_fallback(
        ...
        **kwargs,
        ...
        try:
            # 尝试主函数
            result = await asyncio.wait_for(primary_func(*args, **kwargs), timeout=config.timeout)
            return result

        except (asyncio.TimeoutError, Exception) as e:
            logger.warning(f"Primary operation '{operation_name}' failed, attempting fallback: {e}")
